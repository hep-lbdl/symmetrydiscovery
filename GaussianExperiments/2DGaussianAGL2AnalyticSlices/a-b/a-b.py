...
class MyLayer(Layer):
    ...
    def build(self, input_shape):
        ...
        self._a = self.add_weight(name='x', 
                                    shape=(1,),
    ...
    def call(self, X):
        ...
        d = 1.0
        t = 0.0
        p = -1.0 if d < 0.0 else 1.0
        
        D = tf.math.sqrt(tf.math.abs(d))
        sin = tf.math.sin(t)
        cos = tf.math.cos(t)
        ...
        M = tf.reshape(M, [2, 2])
        return tf.linalg.matmul(X, M) + tf.transpose([self._a, self._b])
...
for j in range(N):
    ...
    a_i.append(generator.layers[-1].get_weights()[0][0])
    b_i.append(generator.layers[-1].get_weights()[1][0])
    # train model
    train(generator, discriminator, gan_model)
    a_f.append(generator.layers[-1].get_weights()[0][0])
    b_f.append(generator.layers[-1].get_weights()[1][0])
    print("A-B Symm Gaussian")
    print("a_i = ", a_i)
    print("b_i = ", b_i)
    print("a_f = ", a_f)
    print("b_f = ", b_f)
